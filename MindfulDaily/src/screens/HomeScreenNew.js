...
const HomeScreen = ({ navigation }) => {
  ...
  // Outdoor tracking
  const [outdoorsDuration, setOutdoorsDuration] = useState('20');
  const [outdoorsActivity, setOutdoorsActivity] = useState('Walk');
  ...
  const toggleActivity = async (activityId) => {
    ...
    // For already completed activities, allow unchecking
  ...
  const saveGratitude = async () => {
    if (gratitudeText.trim()) {
      ...
      const updatedActivities = activities.map(a => 
        a.id === 'gratitude' ? { ...a, completed: true, entries } : a
      );
      setActivities(updatedActivities);
      await storage.saveTodayActivities(updatedActivities);
      await updateStreaks();
                ...
                <View style={styles.activityInfo}>
                  ...
                  {activity.completed && (
                    <Text style={styles.activityDetails}>
                      {activity.id === 'exercise' && `${activity.type} • ${activity.duration} min • ${activity.intensity}`}
                      {activity.id === 'meditation' && `${activity.actualDuration || activity.targetDuration} minutes`}
                      {activity.id === 'gratitude' && `${activity.entries?.length || 0} items`}
                      {activity.id === 'outdoors' && `${activity.activity} • ${activity.duration} min`}
                      {activity.id === 'sleep' && `${activity.hours}h • Quality: ${activity.quality}/10`}
                    </Text>
                  )}
                </View>
              </View>
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <Picker
              selectedValue={outdoorsActivity}
              style={styles.picker}
              onValueChange={setOutdoorsActivity}
            >
              <Picker.Item label="Walk" value="Walk" />
            ...
            <TextInput
              style={styles.input}
              value={outdoorsDuration}
              onChangeText={setOutdoorsDuration}
              keyboardType="numeric"
              placeholder="20"
            />
